...
.melt_mat <- function(x){
  if (is.null(colnames(x)))
    colnames(x) <- 1:ncol(x)
  data.frame(key=rep(colnames(x), each=nrow(x)),
                value=as.numeric(x))
...
as_df.PCA <- function(x, retain, ...){
  ...
  scores_df <- dplyr::select(scores_df, 1:retain)
...
as_df.LDA <- function(x, retain, ...){
  scores_df <- as.data.frame(x$mod.pred$x)
